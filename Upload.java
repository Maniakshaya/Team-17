...
public class Upload extends HttpServlet {
    ...
    ResultSet rs = st.executeQuery("select * from joined where email= '"+a+"' ");
    if(rs.next())
    {
        grop = rs.getString("grop");
    }
                           
                                pstm = con.prepareStatement("insert into upload (file, filename, CDate, cipher,email,skey,grop)values(?,?,?,?,?,?,?)");
                                ...
                                pstm.setString(7, grop);
                                
                                
                            if(kk==1){
                                
                                File f = new File("C:\\Users\\WELCOME\\Desktop\\ramu\\"+fileItem.getName());
                                ...
                                if (i == 1) {
                                    response.sendRedirect("u_upload.jsp?msg1=success");
                                } else {
                                    response.sendRedirect("u_upload.jsp?msgg=failed");
                                }
                                con.close();
                            } catch (Exception e) {
